...
Client.prototype.getClientCache = function() {
	return JSON.parse(JSON.stringify(this._client_cache.getNeighbors()));
}
...
Client.prototype._messageHandler = function(message, remote) {
	...
	switch(data.type) {
		...
		case msgs.types.GROUP_MESSAGE:
			// Messages to a group
			this.emit('client-group-message', data);
			break;
...
// updates the group cache asynchronically
// emits 'group-cache-update' when it's done
...
// function getGroups
// @returns an object of the groups the client has joined
Client.prototype.getGroups = function() {
	return JSON.parse(JSON.stringify(this._groups));
}

// function getGroupCache
// @returns an object of neighboring groups (cached list)
...
Client.prototype._tcpMessageHandler = function(data) {
	...
	switch(data.type) {
		...
		// GROUP_*_OK
		case msgs.types.GROUP_CREATE_OK:
			// We created the group, we join it
