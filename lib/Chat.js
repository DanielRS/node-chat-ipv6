...
Chat.Client = function() {
	...
	this.on('client-message', function(data) {
		if (!self.message_history[data.sender.uuid]) self.message_history[data.sender.uuid] = [];
		self.message_history[data.sender.uuid].push(new ChatMessage(data.sender, data.time, data.extras));
		self.emit('chat-message', sender.uuid);
	});

	this.on('group-message', function(data) {
		if (!self.message_history[data.extras.group.uuid]) self.message_history[data.extras.group.uuid] = [];
		self.message_history[data.extras.group.uuid].push(new ChatMessage(data.sender, data.time, data.extras.message));
