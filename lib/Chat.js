...
Chat.Client = function() {
	...
	this.on('client-message', function(sender, time, message) {
		this._addMessage(sender.uuid, new ChatMessage(sender, time, message));
		self.emit('chat-message', sender.uuid);
	});

	this.on('group-message', function(sender, time, group_uuid, message) {
		this._addMessage(group_uuid, new ChatMessage(sender, time, message));
		self.emit('chat-group-message', group_uuid);
	});
}
...
Chat.Client.prototype.sendGroupMessage = function(uuid, message) {
	...
	this.sendUDP(this.getGroupCache()[uuid].channel, m.toString());
